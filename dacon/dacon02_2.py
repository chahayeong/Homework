...
def modeling(seed):
    ...
    for train_idx, valid_idx in skf.split(data_set[0], data_set[1]):
        ...
        display('='*50)
    display('=== VALIDATION DONE ===')
    display('VALIDATION SCORE : ', np.mean(skf_score))
    p = np.argmax(np.mean(predictions,axis = 0), axis = 1)
    s = np.mean(skf_score)
    sample = pd.read_csv('./dacon/_data02/sample_submission.csv')
    sample['label'] = p
    display(sample.head())
...
sample.to_csv('day0805 ensemble.csv', index= False)
sample.head()